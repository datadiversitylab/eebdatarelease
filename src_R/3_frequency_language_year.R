...
min_year_span = 3
  ...
  # Step 1: Create unique papers dataset
  papers_unique <- df %>%
    select(id, language, Term, publication_year) %>%
    distinct()

  print("=== DESCRIPTIVE STATISTICS ===")
  print("Unique papers by year, language, and term:")
  ...
  # Step 2: Create proportion data by year and term
  ...
  print("\n=== LANGUAGE PROPORTIONS BY YEAR AND TERM ===")
  print(yearly_proportions)

  # Step 3: Statistical Tests for Each Term
  print("\n=== STATISTICAL TESTS BY TERM ===")
  ...
  for (term in terms) {
    ...
    # Test 2: Trend analysis for each language within this term
    cat("\n--- TREND ANALYSIS FOR INDIVIDUAL LANGUAGES ---\n")
  ...
  # Step 4: Overall test across all terms
  print("\n=== OVERALL ANALYSIS ACROSS ALL TERMS ===")

  # Three-way contingency table: Year × Language × Term
  if (length(unique(papers_unique$Term)) > 1) {
    ...
    print("Overall contingency table (Year_Term × Language):")
    print(overall_table)

    if (min(overall_table) >= 5 && nrow(overall_table) > 1 && ncol(overall_table) > 1) {
  ...
  # Plot 1: Language proportions over time by term
